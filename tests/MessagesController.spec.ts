...
describe('MessagesController', () => {
  ...
  it('Method showNotification works properly', () => {
    ...
    expect(console.log).toBeCalledWith(`%c ${testMessage} `, "background: #222; color: #bada55");
  });

  it('Method showNotificationsHistory works properly', () => {
    messagesController.showNotificationsHistory();
    expect(console.log).toBeCalledWith(`${new Date().toJSON().slice(0,19).split('T').join(' ')} ${testMessage}`);
  });
